...
// quote форматирует строковый литерал.
//
// Поведение:
//   - короткие однострочные строки → экранируем перевод строки как \n
//   - многострочные строки (содержат хотя бы один '\n') → выводим
//     "сырыми": реальный перевод строки попадает в исходник.
//     Лексер Ясного принимает обе формы, поэтому смысл программы
//     не меняется, а читаемость сохраняется.
//   - строки с интерполяцией обрабатываются отдельно (внутри {})
func quote(s string) string {
	...
	// Многострочная строка без интерполяции — оставляем как есть.
	// Это критично для SQL, шаблонов писем, JSON-литералов и т.п.,
	// где разработчик специально оформил строку в несколько строк.
	if !hasInterpolation && strings.Contains(s, "\n") {
		// Проверяем что не содержит ` или " подряд — иначе придётся экранировать.
		// Двойные кавычки внутри уже корректно работают если их нет на стыках.

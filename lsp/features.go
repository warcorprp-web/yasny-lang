...
var builtinFunctions = []struct {
	...
	{"мин", "функция(...чисел | массив)", "Минимум: мин(a,b,c) или мин([1,2,3])"},
	{"макс", "функция(...чисел | массив)", "Максимум: макс(a,b,c) или макс([1,2,3])"},
}

// methodSuggestions — методы, доступные через obj.метод(...).
// Любая встроенная функция f(объект, args) работает как объект.f(args).
// Этот список используется для completion после точки.
var methodSuggestions = map[string][]struct {
	Name   string
	Detail string
	Doc    string
}{
	"ARRAY": {
		{"длина", "() → число", "Количество элементов"},
		{"добавить", "(элемент)", "Добавить в конец (мутирует)"},
		{"удалить", "(индекс)", "Удалить по индексу"},
		{"вставить", "(индекс, элемент)", "Вставить по индексу"},
		{"содержит", "(элемент) → да/нет", "Есть ли элемент"},
		{"найти", "(условие) → элемент", "Первый элемент по условию"},
		{"найти_индекс", "(условие) → индекс", "Индекс первого совпадения"},
		{"фильтр", "(условие) → массив", "Отфильтровать"},
		{"преобразовать", "(функция) → массив", "Преобразовать каждый"},
		{"свернуть", "(функция, начальное)", "Свернуть в одно значение"},
		{"сортировать", "() → массив", "Отсортировать (новый массив)"},
		{"реверс", "() → массив", "Развернуть"},
		{"уникальные", "() → массив", "Только уникальные"},
		{"сумма", "() → число", "Сумма элементов"},
		{"взять", "(N) → массив", "Первые N элементов"},
		{"пропустить", "(N) → массив", "Без первых N элементов"},
		{"все", "(условие) → да/нет", "Все элементы по условию"},
		{"любой", "(условие) → да/нет", "Хотя бы один по условию"},
	},
	"STRING": {
		{"длина", "() → число", "Количество символов"},
		{"верхний", "() → строка", "В верхний регистр"},
		{"нижний", "() → строка", "В нижний регистр"},
		{"обрезать", "() → строка", "Убрать пробелы по краям"},
		{"разделить", "(разделитель) → массив", "Разбить на массив"},
		{"заменить", "(старое, новое) → строка", "Заменить все вхождения"},
		{"содержит", "(подстрока) → да/нет", "Есть ли подстрока"},
		{"начинается_с", "(префикс) → да/нет", "Начинается с"},
		{"заканчивается_на", "(суффикс) → да/нет", "Заканчивается на"},
		{"повторить", "(N) → строка", "Повторить N раз"},
		{"подстрока", "(от, до) → строка", "Срез по индексам"},
	},
	"HASH": {
		{"длина", "() → число", "Количество ключей"},
		{"ключи", "() → массив", "Все ключи"},
		{"значения", "() → массив", "Все значения"},
		{"содержит", "(ключ) → да/нет", "Есть ли ключ"},
	},
}
...
func (s *Server) completion(req *jsonrpc2.Request) (interface{}, error) {
	...
	if dotIdx := strings.LastIndex(prefix, "."); dotIdx >= 0 {
		...
		// Не модуль — значит obj.метод(). Без анализа типов не можем
		// точно знать тип obj. Показываем объединённый список методов
		// из всех типов (Array/String/Hash). Дубликаты убираем.
		items := []interface{}{}
		seen := map[string]bool{}
		for _, methodList := range methodSuggestions {
